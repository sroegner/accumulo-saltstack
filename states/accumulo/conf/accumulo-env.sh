...
if [ -z "$HADOOP_HOME" ]
then
   test -z "$HADOOP_PREFIX"      && export HADOOP_PREFIX={{ pillar['HADOOP_HOME'] }}
else
...
test -z "$JAVA_HOME"             && export JAVA_HOME={{ pillar['JAVA_HOME'] }}
test -z "$ZOOKEEPER_HOME"        && export ZOOKEEPER_HOME={{ pillar['ZOOKEEPER_HOME'] }}
...
test -z "$ACCUMULO_TSERVER_OPTS" && export ACCUMULO_TSERVER_OPTS="${POLICY} -Xmx1000m -Xms1000m "
